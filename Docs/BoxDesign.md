...
> 内含若干隐藏 Pixel 并标注容量；当可用格（本体 + 相邻 + 连通）≥ 容量时开箱，把隐藏 Pixel 整体规划
> 释放到候选格（优先级：本体 > 相邻 > 连通距离），确保同色像素各自 4 方向连通，
> 分两段动画（本体外 Pixel 依次 Jump、本体内 Pixel 原地站起、箱子放大缩小消失）释放为普通可点击 Pixel。
>
> 相邻仅按上下左右 4 方向（不含四角）；相邻的连通空格也可占用。容量以 `colorIds`（内容数）为准（详见 §5.1）。
> 位置分配采用**整体规划**（详见 §7.2）：按颜色分组、同色像素落在一组 4 方向连通的候选格上。
...
4. 当**可用格 ≥ `capacity`** 时开箱，把隐藏 Pixel 整体规划释放到「本体 + 相邻 + 连通」空格：
   候选格按优先级（本体 > 相邻 > 连通距离）排序，颜色按 `colorIds` 分组，同色像素落在各自 4 方向连通的候选格上。
...
- 开箱触发条件明确：可用格（本体 + 相邻 4 方向 + 连通空格）≥ 容量。
- 位置分配优先级明确：本体 > 相邻 > 连通格（按距离），且同色像素各自 4 方向连通。
...
| **相邻格子（Adjacent Cell）** | 与箱子矩形边界邻接的格子：仅上下左右 4 方向（**设计约定：不含四角**）。需为空（无像素、非墙/管/箱） |
| **连通空格（Connected Empty）** | 从相邻空格出发、**只经上下左右 4 方向**的"空"格 BFS 扩散可达的所有空格（对角不算连通；**可占用**，优先级最低） |
| **可用格（Available Cell）** | 开箱时可摆放 Pixel 的格子集合 = 本体格 + 相邻格 + 连通空格 |
| **隐藏 Pixel（Hidden Pixel）** | 箱子内尚未释放的 PixelItem，哨兵坐标 `gridX=gridZ=-1`，不进入 `grid` |
| **容量（Capacity）** | 箱子容量 = 隐藏 Pixel 数量 = 开箱触发阈值 = **`colorIds` 数量（内容数）** |
...
public class BoxItem : MonoBehaviour
{
    ...
    // 内容
    public int capacity;                 // 容量 = 隐藏 Pixel 数量 = 触发阈值 = colorIds 数量（内容数）
    public int[] colorIds;               // 每个隐藏 Pixel 的颜色 ID，长度 == capacity

    // 行为
    // （相邻固定 4 方向不含四角；连通空格可占用、优先级最低，已不再作为可配字段）
...
```
可用格 = 箱子本体格子 ∪ 相邻空格（直接相邻，仅上下左右 4 方向）∪ 连通空格
触发条件：可用格数量 ≥ capacity
```

- **相邻空格**：与箱子矩形边界邻接、且 `IsEmpty`（无像素、非墙/管/箱）的格子；仅上下左右 4 方向（不含四角）。
- **连通空格**：从相邻空格出发、只经 4 方向空 BFS 可达的空格（对角不算连通）；**可占用**，优先级最低。

### 7.2 位置分配（整体规划，同色连通）

对 `capacity` 个隐藏 Pixel，从可用格（本体 + 相邻 + 连通）中分配位置，**整体规划、确保同色像素各自连通**：

1. **候选格优先级**：本体格（0）> 相邻格（1）> 连通格（2 + 到相邻格的距离）。距离越近越优先。
2. **颜色分组**：隐藏 Pixel 按 `colorId` 分组，组按数量降序（大块优先占高优先级格）。
3. **回溯求全连通解**：把每种颜色分配到一组 4 方向连通的候选格（数量 = 该颜色像素数、互不重叠）。
   只要存在「所有颜色各自连通」的完整解，就优先取它——避免贪心先占走某块、把剩余候选格割裂，
   导致本该连通的颜色被截断。
...
候选格 = 本体 ∪ 相邻 ∪ 连通，按优先级打分
按颜色分组，颜色组按数量降序：
    回溯：从剩余候选格中枚举该颜色所有「大小 = n 的 4 方向连通块」（按优先级排序），逐一尝试并递归下一颜色；
          后一颜色无解则回溯，给前一颜色换一个稍差的连通块重试（受预算约束）
若回溯找到完整解 → 采用（同色像素落在各自连通的格子上）
否则 → 贪心兜底（见下）
```

回溯要点：

- **连通块枚举**：以「块内优先级最高格」为种子、只朝优先级更低的格子生长，保证每个连通块唯一枚举；结果按优先级（分数和升序）排序。
- **预算约束**：回溯节点数与每色候选块数均有上限（`BacktrackBudget` / `MaxBlocksPerColor`），防止极端关卡穷举爆炸；超预算即视为无解，退回贪心兜底。
- **优先级不回归**：回溯对最大颜色枚举出的第一个连通块，即原贪心 BFS 生长出的那块（二者都「始终取优先级最高的边界格」），
  因此**贪心能连通时回溯结果与原贪心一致**；只有贪心会截断时，回溯才用最小偏差的替代块换回连通。
- 优先级仍保证先占本体、再相邻、最后才向连通空域外扩。

**贪心兜底**（仅当回溯无解/超预算时）：

```
每种颜色从剩余候选格中选优先级最高的种子，BFS 生长出 n 个连通格 → 分配给该颜色
从剩余候选格移除这些格；若剩余候选格被墙/管割裂、无法连通容纳该颜色，按优先级补齐（此时该颜色可能不连通）并告警
```

- 动画顺序（本体外依次 Jump）仍按 (row, col) 排序，与分配结果无关。
...
- **容量 = `colorIds` 数量（内容数）**；`BoxItem.ComputeCapacity()` 计算的是「本体 + 相邻 4 方向」的紧邻基础容量，仅作编辑器参考/兜底。
- 运行时 `PixelGroup.SpawnBox` 以 `colorIds` 数量作为容量（不再覆盖为周围环境值）。
- 编辑器 `BoxItemEditor` Inspector 显示"紧邻基础容量"，`capacity` 超过它时提示（超出部分占用连通空格），并提供 **「按周围环境重算 capacity」** 按钮。
- 创建向导 `BoxCreateWizard` 相邻固定 4 方向（不含四角），`capacity` 默认取紧邻基础容量值（可手动增容以占用连通格）。
...
| 可用格 > capacity | 正常（候选格含连通空格，可能多于容量）；只占用优先级最高、且同色连通的一批格子，多余格子留空 |
...
| 对角相邻 | 固定不纳入（相邻仅 4 方向）；连通空格可占用（4 方向 BFS，优先级最低） |
| capacity 与周围环境不一致 | capacity 以 `colorIds` 数量为准；编辑器 Inspector 提示并提供「重算」按钮（见 §11.2） |
...
3. **位置分配的随机粒度** —— ✅ **已定**：改为**整体规划**（不再顺序/随机）。按颜色分组、同色像素落在 4 方向连通的候选格上，见 §7.2。
...
- [ ] 满足触发条件（可用格 ≥ 容量）时开箱：候选格按优先级（本体 > 相邻 > 连通距离）排序，同色像素落在各自连通的候选格上。
...
- [ ] 开箱动画期间失败判定不误触发。
- [ ] 相邻固定 4 方向（不含四角）生效；连通空格可占用（优先级最低）。
- [ ] 编辑器拒绝长宽 < 2 的箱子。
