...
该像素上已有生气表情在播            → 返回（上一张还没播完，忽略本次）
否则 → 在被点像素上播（跟随模式）
```

两条与其它来源不同：

- **必出**：不掷概率（`angryChance` 参数已删除）。点一下就有反馈。
- **没有全局 CD**：抑制改成**逐像素**的——同一个像素上一张生气还没播完就忽略本次。
  这样连点在**同一颗**头上不会叠脸，但点**不同**像素（哪怕同一组）各自照常出，
  而原来那种「一次没中就哑火 3 秒」的手感也一并消失。

> **点谁谁生气**：不再从连通组里随机抽一个——被点的那一颗才是玩家真正交互的对象，表情落在它头上才有因果感。
>
> 判定用的是「该锚点上是否已有 tag 匹配的表情在播」（`HasEmoji(anchor, angryTag)`），
> 也就是**按 tag 判**，不区分这张生气来自点击 / 插队 / 排队。三者共用同一个 emoji 与同一个挂点，
> 玩家看到的就是「这颗头上已经有生气脸了」，再叠一张没有意义。
...
7. **三条生气来源只共用 emoji**：判定与节流相互独立——点击受阻**必出 + 逐像素抑制**（无概率、无全局 CD），插队是概率 + 独立全局 CD，排队靠检测间隔节流（无 CD）。
8. **CD（仅插队还有）只在真正播放后起算**：未命中不进 CD，避免「一次没中就哑火一整个 CD」，同时播放频率仍被 CD 上限住。
...
20. **点击受阻改成「点谁谁生气」，不再从连通组里随机抽**：被点的那一颗才是玩家真正交互的对象，表情落在它头上才有因果感（随机抽时可能落在组里离点击最远的一颗上，读起来像「随机撒的表情」）。被点像素若没配 `emojiNode` 就直接不播（没有回退到组内随机——那会让语义又变回不确定）。
21. **点击受阻必出、且不设全局 CD**：点一下就该有反馈，掷概率会让「点了没反应」变成常态。抑制改成**逐像素**判「这张脸还在不在播」（`HasEmoji`），比全局 CD 更贴合直觉——连点同一颗头不叠脸，点不同像素则各自照常出。随之删掉了 `angryChance` / `angryCooldown` / `_angryReadyTime`（不再有任何读取点，留着会变成调了没反应的陷阱参数）。
22. **逐像素抑制按 tag 判、不区分生气来源**：三条来源共用同一个 emoji 与同一个挂点，对玩家而言「这颗头上已经有生气脸了」，再来一张没有意义；为此给 Booking 加一个「来源」字段只为区分三种生气，收益不抵复杂度。
23. **排队与插队仍是随机取候选中的一个**：这两条没有「玩家当时点的是谁」这个锚点（排队是定时检测、插队是别人上带触发），随机取是唯一自然的选法。
24. **生气表情在缓冲区 → 传送带这一段保留，只在「上车」时移除**：原来是在 `OnSlotPassedEntry`（上带那一刻）就收掉，等于怒气在排队时刚攒出来、一上带就没了。改成在 `OnLeave`（离开传送带、交给容器那一刻）收，压在带上的那一段路程里表情一直挂着，观感更连贯；`OnLeave` 正是犯困表情的回收点，两者合并成一处。
25. **不做「一波多点」**：曾按需求实现过（候选数 / N 定上限 + 连锁概率 + 累积延迟 + 延时复查谓词），后按需求整体回退——三条生气来源都只播一个表情。回退时一并删掉了 `AngryWave`、`clickAngryWave` / `queueAngryWave`、`PlayAngryWave` / `ScheduleAngryEmoji` / `PlayAngryEmojiIfStillValid`，以及为延时复查而加的 `stillValid` 谓词、`GameController.IsStillInGrid`、`ConveyorBeltZone.IsStillWaiting`、`CrowdBufferZone.IsWaiting`。
...
5. **点击受阻生气**：点击无法移出的连通组 → **必出**（不掷概率），在**被点的那一颗**上播 `EmojiAngry`（**不是**组内随机一颗）。
   分项验证：紧接着再点**同一颗**（表情还在播）→ 不叠第二张；点**同组另一颗** → 那一颗照常出；
   等它播完（或超时）后再点同一颗 → 又出；同一像素后来被成功移出时表情被收掉。
